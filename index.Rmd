...
num <- length(unique(games$Game))
cat <- length(unique(games$Category))
catnames <- unique(games$Category)

```
- Draws from a database of `r num` games, many of which are highly ranked on game websites such as [BoardGameGeek](www.boardgamegeek.com).
- Contains games in `r cat` categories, including abstract thinking games, children's games, cooperative games, family games, strategy games, party games, and war-themed games.
